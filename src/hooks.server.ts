...
const handleCSP: Handle = async ({ event, resolve }) => {
	...
	response.headers.set(
		...
		`default-src 'self'; script-src 'self' 'nonce-${nonce}' 'strict-dynamic'; style-src 'self' 'nonce-${nonce}' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self'; frame-src 'self' data:; child-src 'self' data:; connect-src 'self' https://api.getdash.ai; frame-ancestors 'none'; form-action 'self'`
	);
