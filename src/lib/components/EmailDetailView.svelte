...
<div
  class="h-full w-full min-w-[740px] max-w-[1200px] rounded-lg bg-primary-container"
>
  {#if email}
    <!-- Header -->
    <header
      class="flex h-[60px] items-center gap-2 border-b border-primary-gray px-3"
    >
      <div class="mx-[24px] flex gap-[24px]">
        ...
        <EmailButton
          onclick={() => email && removeEmail(email)}
          ariaLabel="Delete"
          title="Delete"
        >
          <Trash />
    ...
    <div class="no-scrollbar relative grid max-h-[850px] max-w-[1200px] p-4">
      ...
      >
        <BackgroundGradient
          className="rounded-md max-w-sm px-4 py-2 bg-primary-black "
        >
          <button
